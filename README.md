...
**核心特性：**
- 🔍 **提示搜索**：支持搜索提示词名称和提示词内容，快速查找定位提示词
...
## 版本历史
- v2.5.2 （2025-07-06）
  - 尝试修复界面布局面板的多系统兼容性BUG，没有测试环境，不确定是否有效。

- v2.5.1 （2025-07-02）
  - 提示词管理面板现在已经完美嵌入大部分官方CLIP节点与部分第三方CLIP节点（可提交issue补充）
  - 提示词管理面板现在已经完美嵌入ComfyUI-Easy-Use与ComfyUI-Fast-Use
  - 修复了部分已知问题

- v2.5.0 （2025-06-23）
 - 对提示词管理面板的UI和交互做了完全重构，多了很多新特性：
   -  1丶标签颜色可以随意设置更改，可以批量编辑标签颜色。
   -  2丶加入了独立的标签管理系统
   -  3丶搜索功能更加强大现在支持标签+文字双重搜索
   -  4丶提示词与标签数据实时同步本地文件，不再是缓存在浏览器。
   -  5丶导出提示词现在会导出你的所有数据包括标签，让你在多台电脑中始终可以共享一套提示词模板提示词数据永不丢失

 - 完善界面布局面板的自定义颜色功能，现在有了单独的颜色预设面板

- v2.4.0  （2025-06-20）
  -  **提示词管理器新功能**：
    - 新增智能备份功能，所有提示词操作自动保存到插件的extensions\Prompt_Manager\prompts.json文件中
     （也可以通过修改json文件来反向修改前端提示词，前后端数据现在统一通过一个json文件来同步）
